...
test_that("makecsv with no auth", {
    with_mock(
        "RCurl::url.exists" = function(...) TRUE,
        "RCurl::getURL" = function(url, ...){
            expect_equal(url, "http://test.org/makecsv?sourceurl=testurl&experimentName=testname&file=default.csv")
            "column1\tcolumn2\n1\t2\n"
        },
...
test_that("makecsv with auth", {
    options(Rexperigen.experimenter = "aa")
    options(Rexperigen.password = "bb")
    with_mock(
        "RCurl::url.exists" = function(...) TRUE,
        "RCurl::getURL" = function(url, username, password, ...){
            expect_equal(url, "http://test.org/digest/makecsv?sourceurl=testurl&experimentName=testname&file=demographics.csv")
